...
Stop the interactive dev, preview or sweeper server **in the checkout you are checking**; a server in another workspace is unrelated and is not a reason to wait. E2E starts its own server and shuts it down. Within one workspace, checks acquire exclusive access automatically: typecheck, build and test suites queue instead of overlapping.
...
The wrapper forwards interruption signals to its child process group on macOS/Linux and releases its slot on normal exit. Leases live in `assubki-books-checks/leases.sqlite` under the system temporary directory, recording the owning PID, checkout and weight. SQLite transactions admit the whole job atomically, including exclusive checkout ownership. A slot whose owner is gone is reclaimed automatically, so a force-kill or reboot no longer leaves a lock for somebody to clear by hand. Invoking Astro or the test scripts directly bypasses the guard.
