...
await check('R01 restock races overlapping sibling holds and oversubscribes volume',async()=>{
 ...
 await app.setBook(request(b,{action:'restock',sets:'1'}));assert.equal(row('SELECT MIN(have) n FROM book_set_stock').n,1);assert.equal(row('SELECT SUM(reserved) n FROM books WHERE set_from=1').n,2);
});
await check('R02 organiser UI payload changes own line instead of selected member',async()=>{
 ...
 const source=readFileSync(join(root,'src/pages/basket.astro'),'utf8');assert.ok(source.includes('name: member.name'));assert.ok(source.includes('line.addedBy === member!.name || member!.organiser'));
 // Exact current UI request identity for clicking Alice's quantity to 2.
 await app.setGroupLine(g.code,g.ownerToken,b,2,'Owner','owner-token');const lines=(await app.getGroup(g.code,g.ownerToken)).lines;
 assert.equal(lines.find(l=>l.addedBy==='Alice').qty,1);assert.equal(lines.find(l=>l.addedBy==='Owner').qty,2);
});
await check('R03 legacy membership generates a different key on each read',async()=>{
 const legacy=JSON.stringify({code:'TEST',token:'shared',name:'Alice',organiser:false});globalThis.localStorage={getItem:()=>legacy};
 const a=app.readGroup(),b=app.readGroup();assert.notEqual(a.memberToken,b.memberToken);delete globalThis.localStorage;
});
...
await check('R06 same-origin URL with double-slash path still redirects externally',async()=>{
 ...
 assert.equal(r.status,302);assert.equal(r.headers.get('location'),'//evil.invalid');assert.equal(new URL(r.headers.get('location'),'https://example.invalid').origin,'https://evil.invalid');
});
await check('R07 product-page group additions omit the now-required member token',async()=>{
 const b=book(null,0,4),g=await app.createGroup('Owner','test@example.invalid');
 const source=readFileSync(join(root,'src/pages/book/[slug].astro'),'utf8');assert.ok(source.includes('JSON.stringify({ code: member.code, token: member.token, name: member.name, bookId, qty: total })'));
 const r=await app.groupLine({request:new Request('https://example.invalid',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:g.code,token:g.token,name:'Alice',bookId:b,qty:1})})});
 assert.equal(r.status,400);assert.equal((await app.getGroup(g.code,g.ownerToken)).lines.length,0);
});
