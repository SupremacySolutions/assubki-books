...
await check('Closed R04 stale acknowledgement cannot restore read badge',async()=>{
 ...
 await app.markRead(o.id,'owner',z);assert.equal(row(`SELECT unread_for_owner n FROM orders WHERE id=${o.id}`).n,0);await app.markRead(o.id,'owner',a);assert.equal(row(`SELECT unread_for_owner n FROM orders WHERE id=${o.id}`).n,0);assert.equal(row(`SELECT read_cursor_owner n FROM orders WHERE id=${o.id}`).n,z);
});
...
await check('Closed R08 refused uneven-pool restock leaves listing stock intact',async()=>{
 ...
 assert.equal(row(`SELECT stock FROM books WHERE id=${part}`).stock,2);
 // The full-set hold is then released; one physical volume 2 is available.
 sql(`UPDATE books SET reserved=0 WHERE id=${b}`);
 assert.equal((await order([part])).length,1);
});
await check('Read cursors are independent and preserve later messages',async()=>{
 const b=book(null,0,1),[o]=await order([b]);
 const a=await app.postMessage({orderId:o.id,sender:'owner',via:'web',body:'A'}),z=await app.postMessage({orderId:o.id,sender:'owner',via:'web',body:'B'});
 await app.markRead(o.id,'customer',z);await app.markRead(o.id,'customer',a);
 assert.equal(row(`SELECT unread_for_customer n FROM orders WHERE id=${o.id}`).n,0);
 assert.equal(row(`SELECT read_cursor_owner n FROM orders WHERE id=${o.id}`).n,0);
 await app.postMessage({orderId:o.id,sender:'owner',via:'web',body:'C'});await app.markRead(o.id,'customer',a);
 assert.equal(row(`SELECT unread_for_customer n FROM orders WHERE id=${o.id}`).n,1);
});
