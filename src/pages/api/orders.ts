...
export const POST: APIRoute = async ({ request, url, locals }) => {
   ...
   */
  let placed: Awaited<ReturnType<typeof createCheckout>>;
  try {
    placed = await createCheckout({
      name,email,phone:phone||null,fulfilment:fulfilment as 'delivery'|'collection',address,
      addressParts:fulfilment==='delivery'?parts:null,paymentPreference:paymentPreference||null,
      notes:finalNotes,items:finalItems,
    });
  } catch(err) {
